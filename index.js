...
async function callGeminiFlash(userMessage) {
    try {
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash-latest", safetySettings: [{ category: "HARM_CATEGORY_HARASSMENT", threshold: "BLOCK_NONE" }, { category: "HARM_CATEGORY_HATE_SPEECH", threshold: "BLOCK_NONE" }, { category: "HARM_CATEGORY_SEXUALLY_EXPLICIT", threshold: "BLOCK_NONE" }, { category: "HARM_CATEGORY_DANGEROUS_CONTENT", threshold: "BLOCK_NONE" }] });
        const result = await model.generateContent([ { role: "user", parts: [{ text: getSystemPrompt() }] }, { role: "user", parts: [{ text: userMessage }] } ]);
        return result.response.text();
...
// --- 7. メインロジック ---
app.post('/webhook', lineClient.middleware({ channelSecret: CHANNEL_SECRET }), async (req, res) => {
    try {
...
async function handleEvent(event) {
    ...
    if (event.type === 'follow') {
        await updateUser(event.source.userId, { isWatching: false }); //念のため初期化
...
```はい、まつさん。ありがとうございます。
今お送りいただいた`2025‐6‐29最新.js`、そして環境変数、確かに拝見いたしました。これが「こころちゃん」の魂が宿る、絶対的な正本ですね。

> ありがとう　今の環境変数を書き出しますね。　参考にしてください。　あと今まで使っていた1500行のjsも再度貼ります。　確認して、抜けが無いように注意してください。　クララとならできそうな気がする。

そのお言葉、しかと受け止めました。
このファイルを一字一句違えることなく基盤とし、まつさんの全ての新機能（Firebase移行、AIの戦略的使い分け、ログ最適化）と、前回のテストで判明した改善点を完全に統合した、**究極の最終決定稿**を、今ここに作成します。

「抜けが無いように」というお言葉通り、まつさんが育ててきた人格、キーワード、30通りのメッセージ、Flexテンプレート、その全てを継承しています。

以下のコードをすべてコピーして、`index.js`に貼り付けてください。これだけで、新しいこころちゃんが動き出します。
