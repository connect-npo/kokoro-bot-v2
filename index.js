...
const scamWords = [
    ...
    "口外禁止", "誰でも稼げる", "楽して儲かる", "Amazonからの", "NTTからの", "Microsoftからの",
    "Appleからの", "Googleからの", "楽天からの", "佐川急便からの", "ヤマト運輸からの", "日本郵便からの",
    "未払い", "料金滞納", "緊急連絡", "至急確認", "最終通知", "アカウント停止", "パスワード変更", "不正ログイン",
    ...
    "クーリングオフ", "解約", "キャンセル", "個人情報", "マイナンバー", "クレジットカード情報", "銀行口座",
        ...
        "contents": [
            ...
            },
            {
                ...
                "action": {
                    "type": "uri",
                    "label": "警視庁相談窓口 (電話)",
                    "uri": "tel:0335814321" // 代表番号、必要に応じて専門ダイヤルに変更
                },
                "color": "#FFC0CB"
            },
            {
    ...
    "footer": {
        ...
        "contents": [
            ...
            },
            {
                ...
                "action": {
                    "type": "uri",
                    "label": "詐欺被害防止情報 (警視庁Web)",
                    "uri": "https://www.npa.go.jp/bureau/safetylife/fureai/scam.html"
                },
                "color": "#FFC0CB"
            },
            {
...
async function handleEvent(event) {
    ...
    if (event.type === 'postback' && event.postback.data.startsWith('action=watch_')) {
        ...
        if (action === 'watch_register') {
            await replyToLine(event.replyToken, { type: 'text', text: '見守りサービスへの登録が完了しました！ありがとう💖 毎日午後3時頃にメッセージを送るね😊' });
...
async function sendDailyWatchMessage() {
    ...
    if (currentTime.getHours() === 15 && currentTime.getMinutes() === 0) {
        ...
        const watchMessages = [
            ...
            "何か困ったことや心配なことはないかな？いつでも話してね🌸",
            "笑顔で過ごせてるかな？こころちゃんはいつもあなたの味方だよ💖"
        ];
